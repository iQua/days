...
| 2026-08-09 | `T22a: Align final gates with exact scheduling` | (d) | Updated the service-start validation fixtures to assert checked integer-ceiling deadlines and made the two standalone TCP examples request reports using their allocated sink IDs rather than the pre-E5 synthetic-source allocation count. | Ordered T7 final-gate failures exposed after the exact scheduling and byte-driven TCP changes. |
| 2026-08-09 | `T22a: Fix legacy fast retransmit semantics` | (d) | Retransmitted the missing segment only on duplicate ACK three while retaining later-duplicate window inflation and fresh-data admission; added an E3-only tripwire for the fast-retransmit path. | The `LEGACY FAST-RETRANSMIT DEFECT — FIX AUTHORIZED` registry entry in `days-gpu/plans/p12-opening-plan.md`. |
