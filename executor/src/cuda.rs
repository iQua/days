...
/// Builds a compaction plan for a class-homogeneous range of streams.
///
/// Stream metadata stores word offsets because the three stream classes have different physical
/// record widths. Entity indices in the returned plan are local to the supplied stream range.
...
fn decode_event_header(record: &[u64], kind: EventKind) -> Result<Event, CudaError> {
    Ok(Event {
        key: EventKey {
            time_ns: record[0],
    ...
    for (stream, capacity) in capacities.iter().copied().enumerate() {
        ...
        offset = offset
            .checked_add(capacity.checked_mul(record_words).ok_or_else(|| {
                CudaError::Validation("device stream arena size overflows usize".into())
            })?)
            .ok_or_else(|| {
                CudaError::Validation("device stream arena size overflows usize".into())
            })?;
