...
/// T20l fix 2: a *successful* attempt must copy back the live regions, not the arena.
///
/// T20l phase 1 §4.2 measured the frontier's successful attempt reading 2,394,309,899 words
/// (19.15 GB) to recover 487,958 pending events and 2,931,779 packet descriptors, and §8 recorded
/// that the meta planes bounding those live records are already on the device before the copy
/// starts. The property is a *quantity* — how many words `finish` asks for — so the test is a
/// counter comparison, with the pre-fix readback as the denominator:
/// `last_plane_words_for_testing` is exactly what the old `finish` read on every successful
/// attempt, because it read every plane in full.
///
/// The fixture has known live counts and they are asserted, so the ratio cannot be satisfied by a
/// compaction that drops records: 6 LPs, 3 flows, 50 packets, 51 live event records at the end.
///
/// | build | plane words | readback words | ratio |
/// | --- | ---: | ---: | ---: |
/// | RED — counters added, compaction not applied | 11,604 | 11,606 | **1.00** |
/// | GREEN — compaction applied | 11,604 | 2,552 | **4.55** |
///
/// RED fails on the strict inequality, because reading every plane makes the two counts equal —
/// its two extra words are fix 1's single-word control probes at the wave boundary. Both arms
/// satisfy every live-count assertion below, which is what makes the comparison a compaction
/// measurement rather than a correctness one.
/// The ratio is modest here only because a 6-LP fixture is dominated by its fixed per-LP planes;
/// at the RQ9 frontier the same mechanism removes 691,104,103 words of elided planes alone.
#[cfg(feature = "metal-test-hooks")]
#[test]
fn metal_readback_copies_the_live_regions_and_not_the_arena() {
    ...
    let readback_words = days_executor::metal::take_readback_words_for_testing();
    ...
    assert_metal_full_result_matches_scalar(&metal.result, &scalar, "compacted Metal readback");
    // The fixture's live counts, pinned so the ratio below cannot be met by dropping records.
    assert_eq!(
        metal.result.pending_events.len(),
        51,
        "51 live event records across the FEL and stream arenas"
    );
    assert_eq!(
        metal.result.resident_packets.len(),
        50,
        "the fixture's three flows carry 3 + 7 + 40 packets"
    );
    assert_eq!(metal.result.observed_packets.len(), 49);
    assert_eq!(metal.result.departures.len(), 49);
    assert_eq!(metal.result.arrivals.len(), 0);
    ...
    // A margin, not just strictness: elision of the ten planes the decode never reads plus the
    // gathered live regions is worth several times the plan on any fixture with real arenas.
    assert!(
        readback_words.saturating_mul(3) < plane_words,
        "the compacted readback must be a small fraction of the arena, not a trim: it copied \
         {readback_words} of {plane_words} plane words"
    );
}
