...
use days_executor::{P11HorizonPolicy, run_scalar_rounds_with_p11_horizon_policy};
...
#[cfg(feature = "p11-profile")]
#[test]
fn p11_profile_decomposes_scalar_and_cpu_without_changing_results() {
    let image = image(5, 0, 4);
    let scalar = run_scalar_rounds_with_observations(&image, None, ObservationMode::Summary)
        .expect("profiled scalar round run should succeed");
    let canonical = run_scalar_with_observations(&image, None, ObservationMode::Summary)
        .expect("canonical scalar run should succeed");
    assert_eq!(scalar.result, canonical);

    let scalar_events = scalar
        .rounds
        .iter()
        .map(|round| round.events_processed)
        .sum::<u64>();
    let scalar_profiled_events = scalar
        .rounds
        .iter()
        .map(|round| round.p11_profile.lp.profiled_events)
        .sum::<u64>();
    let same_time = scalar
        .rounds
        .iter()
        .map(|round| round.p11_profile.lp.consecutive_same_time_events)
        .sum::<u64>();
    assert_eq!(scalar_profiled_events, scalar_events);
    assert!(same_time > 0, "the arrival/ready pair shares one timestamp");
    assert!(
        scalar
            .rounds
            .iter()
            .all(|round| round.p11_profile.lp.outbox_key_inversions == 0)
    );
    assert!(
        scalar
            .rounds
            .iter()
            .all(|round| round.p11_profile.lp.outbox_target_inversions == 0)
    );
    assert!(
        scalar
            .rounds
            .iter()
            .any(|round| round.p11_profile.lp.transition_body_ns > 0)
    );
    assert!(
        scalar
            .rounds
            .iter()
            .all(|round| round.p11_profile.round_wall_ns >= round.p11_profile.horizon_ns)
    );
    ...
    .expect("profiled CPU round run should succeed");
    assert_eq!(cpu.result, canonical);
    assert_eq!(
        cpu.rounds
            .iter()
    ...
    assert_eq!(
        cpu.rounds
            .iter()
            .map(|round| round.semantic.p11_profile.lp.profiled_events)
            .sum::<u64>(),
        scalar_events
    );
}
...
#[cfg(feature = "p11-profile")]
#[test]
fn p11_transitive_per_lp_horizon_matches_global_on_uniform_delay() {
    let image = image(40, 0, 4);
    let global = run_scalar_rounds_with_p11_horizon_policy(
        &image,
        None,
        ObservationMode::Full,
        P11HorizonPolicy::Global,
    )
    .expect("global P11 horizon run should succeed");
    let per_lp = run_scalar_rounds_with_p11_horizon_policy(
        &image,
        None,
        ObservationMode::Full,
        P11HorizonPolicy::TransitivePerLp,
    )
    .expect("per-LP P11 horizon run should succeed");

    assert_eq!(per_lp.result, global.result);
    assert!(global.rounds.iter().all(|round| {
        round.p11_profile.horizon_policy == P11HorizonPolicy::Global
            && round.p11_profile.horizon_bound_min_ns == round.p11_profile.horizon_bound_max_ns
    }));
    assert!(per_lp.rounds.iter().all(|round| {
        round.p11_profile.horizon_policy == P11HorizonPolicy::TransitivePerLp
            && round.p11_profile.horizon_ns <= round.p11_profile.round_wall_ns
    }));
}

#[cfg(feature = "p11-profile")]
#[test]
fn p11_transitive_per_lp_horizon_matches_global_and_changes_heterogeneous_round_count() {
    ...
    assert_eq!(per_lp.result, global.result);
    assert_eq!(global.rounds.len(), 16);
    assert_eq!(per_lp.rounds.len(), 3);
    assert!(per_lp.rounds.iter().any(|round| {
        round.p11_profile.horizon_distinct_bounds > 1
            && round.p11_profile.horizon_bound_min_ns < round.p11_profile.horizon_bound_max_ns
    }));
}
