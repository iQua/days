...
/// Capacity overrides change allocation only, not Metal retry semantics or the decoded result.
#[cfg(all(feature = "metal-test-hooks", target_vendor = "apple"))]
#[test]
fn metal_ledger_retry_is_semantic_at_inflated_capacity() {
    let image = tcp_image(TcpCongestionControl::reno(MSS), 2 * MSS);
    let scalar = run_scalar_with_observations(&image, None, ObservationMode::Full)
        .expect("scalar TCP ledger retry oracle must run");
    let capacity_caps = DeviceCapacityCaps {
    ...
    let arm = |inflate: Option<usize>| {
        ...
        .expect("adaptive TCP ledger growth must restart from the immutable image");
        (strict_error, recovered)
    };

    let (strict_error, recovered) = arm(None);
    let (_, large_run) = arm(Some(8_192));
    ...
    assert_device_full_result_eq(&recovered.result, &scalar, "Metal TCP ledger retry run");
    assert_device_full_result_eq(
        &large_run.result,
        &scalar,
        "Metal TCP ledger retry run at the inflated plan size",
    );
    let [retry] = recovered.capacity_retry_trace.as_slice() else {
        panic!(
            "expected one TCP ledger retry, got {:?}",
...
/// Capacity overrides change allocation only, not CUDA retry semantics or the decoded result.
#[cfg(feature = "cuda-test-hooks")]
#[test]
fn cuda_ledger_retry_is_semantic_at_inflated_capacity() {
    let image = tcp_image(TcpCongestionControl::reno(MSS), 2 * MSS);
    let scalar = run_scalar_with_observations(&image, None, ObservationMode::Full)
        .expect("scalar TCP ledger retry oracle must run");
    let capacity_caps = DeviceCapacityCaps {
    ...
    assert_device_full_result_eq(&recovered.result, &scalar, "CUDA TCP ledger retry run");
    assert_device_full_result_eq(
        &large_run.result,
        &scalar,
        "CUDA TCP ledger retry run at the inflated plan size",
    );
    let [retry] = recovered.capacity_retry_trace.as_slice() else {
        panic!(
            "expected one TCP ledger retry, got {:?}",
