...
```bash
RUST_LOG=info cargo run -p days-legacy --features l2_pfc,lean --bin days -- configs/pfc.toml
```
...
```bash
RUST_LOG=info cargo run -p days-legacy --features lean --bin days -- configs/simple.toml
```
