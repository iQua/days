...
description: Seed, duration, threading, mailbox sizing, and CSV reports.
---
...
- Runtime and UI keys are read from shared config structs in `src/topos/config.rs` and the legacy runtime in `legacy/src/topos/topo.rs`.
...
Note: `duration` is parsed by multiple subsystems (topology and UI). For consistency, prefer setting it explicitly.
...
For certain TCP collectives (broadcast, ring allreduce), legacy Days can back TCP flows with actor-managed byte buffers (`legacy/src/flows/app_source.rs`).
