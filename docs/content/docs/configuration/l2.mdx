...
Legacy link-layer behavior is configured under `[link]` and implemented in `legacy/src/l2/`.
...
Fields and defaults (from `Topology::build_pfc_config` in `legacy/src/topos/topo.rs`):
