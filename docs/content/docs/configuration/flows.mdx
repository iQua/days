...
Legacy flow parsing is implemented in `legacy/src/flows/flow.rs`. Flows are end-to-end traffic generators that attach to host switches and send packets through the routed network.
...
Traffic is parsed via `TomlTrafficCharacteristics` (`legacy/src/flows/mod.rs`).
