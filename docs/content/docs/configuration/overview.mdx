...
Days simulations are configured using a single TOML file. The legacy CLI entry point (`legacy/src/main.rs`) expects the path to that file as its only argument:
