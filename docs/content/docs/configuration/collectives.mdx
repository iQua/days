...
- **TCP RingAllReduce**: each directed ring link uses its own app-source buffer, and each generated hop reads a specific byte range from that link-local buffer via an offset/length handle. This models per-hop byte counts for the ring schedule; it does not model payload reduction across hops.
