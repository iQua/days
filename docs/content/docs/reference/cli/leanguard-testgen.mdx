...
Supported campaign protocols are `dcqcn`, `aqm`, `pfc`, `wfq`, `drr`, and
`cubic`.
...
- `--legacy-runner <path>` (optional)
  - Forwarded to `leanguard-run`; point it at the separately built `days-legacy` binary for simulate-and-check campaigns.
