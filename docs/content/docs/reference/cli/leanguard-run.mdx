...
# Build the separate legacy trace generator
cargo build -p days-legacy --features dcqcn,l2_pfc,lean --bin days

# Run simulation + checkers through the explicit test-only handoff
...
- `--legacy-runner <path>` (optional)
  - Path to the separately built `days-legacy` binary used only by `simulate-and-check`.
  - If omitted, `leanguard-run` checks `DAYS_LEGACY_RUNNER`, then a sibling executable named `days`.
