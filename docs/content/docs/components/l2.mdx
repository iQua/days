...
When L2 is compiled in, internal link pipelines use `LinkFrame` (`legacy/src/l2/frame.rs`):
...
`Link` (`legacy/src/l2/link.rs`) is a small model that:
...
PFC is implemented by two per-edge actors (`legacy/src/l2/pfc.rs`):
