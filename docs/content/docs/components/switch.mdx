...
`PacketSwitch` (`legacy/src/switches/switch.rs`) is a demultiplexing switch model:
...
These tables are installed after flow paths are computed in `Topology::route_flows` (`legacy/src/topos/topo.rs`).
