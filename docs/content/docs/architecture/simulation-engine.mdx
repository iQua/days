...
- `PacketSwitch` (`legacy/src/switches/switch.rs`)
- schedulers such as `Port`, `DRRServer`, `WFQServer` (`legacy/src/schedulers/`)
- flow endpoints such as `DistPacketSource`, `TCPPacketSink` (`legacy/src/flows/`)
...
The choice is made in `Topology::new` (`legacy/src/topos/topo.rs`) using `SimInit::with_num_threads(...)`.
...
The entrypoint `Topology::run` (`legacy/src/topos/topo.rs`) performs this wiring and then runs:
