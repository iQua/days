#!/usr/bin/env python3
"""
Calibrate Days simulated Broadcast CCT against NCCL real measurements.

Default behavior is to FIT Broadcast's own (scale, L) per TP:
  - scale: median(t_real/t_sim) over sizes >= 16MB
  - L: median residual over sizes < 16MB

You can alternatively reuse the AllReduce-derived (scale, L).

Inputs (repo root):
  - real_bcast_tp2_time.csv / real_bcast_tp3_time.csv     (Broadcast time_us_mean)
  - sim_bcast_tp2.csv / sim_bcast_tp3.csv                 (Broadcast cct_s)
  - real_tp2_time.csv / real_tp3_time.csv                 (AllReduce time_us_mean; optional for reuse)
  - sim_tp2.csv / sim_tp3.csv                             (AllReduce cct_s; optional for reuse)

Outputs (repo root):
  - compare_bcast_tp2.csv
  - compare_bcast_tp3.csv
  - calib_bcast_tp2_time.svg
  - calib_bcast_tp3_time.svg
"""
...
def plot_time(out_path: Path, points: list[Point], scale: float, L: float, title: str) -> None:
    # Minimal SVG log2(size) vs log10(time) plot (no matplotlib dependency).
...
def reuse_allreduce_scale_L(repo: Path, tp: int) -> tuple[float, float]:
    real = read_real(repo / f"real_tp{tp}_time.csv")
    sim = read_sim(repo / f"sim_tp{tp}.csv")
    pts = join_points(real, sim)
    if len(pts) != len(NCCL_SIZES):
        raise RuntimeError(f"tp{tp}: expected {len(NCCL_SIZES)} allreduce points, got {len(pts)}")
    scale = median_scale(pts)
    L = fit_L(pts, scale)
    return scale, L


def calibrate_bcast_one(repo: Path, tp: int, mode: str) -> None:
    real_path = repo / f"real_bcast_tp{tp}_time.csv"
    sim_path = repo / f"sim_bcast_tp{tp}.csv"
    out_compare = repo / f"compare_bcast_tp{tp}.csv"
    out_plot = repo / f"calib_bcast_tp{tp}_time.svg"

    real = read_real(real_path)
    sim = read_sim(sim_path)
    pts = join_points(real, sim)
    if len(pts) != len(NCCL_SIZES):
        raise RuntimeError(f"tp{tp}: expected {len(NCCL_SIZES)} bcast points, got {len(pts)}")

    if mode == "fit_bcast":
        scale = median_scale(pts)
        L = fit_L(pts, scale)
        mode_label = "fit"
    elif mode == "reuse_allreduce":
        scale, L = reuse_allreduce_scale_L(repo, tp)
        mode_label = "reused"
    else:
    ...
    plot_time(
        ...
        scale,
        L,
        title=f"Broadcast TP{tp} ({mode_label} scale={scale:.3f}, L={L*1e6:.1f}us)",
    )

    print(
        f"bcast tp{tp}: {mode_label} scale={scale:.4f}, L={L*1e6:.1f}us -> {out_compare.name}"
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--mode",
        type=str,
        choices=["fit_bcast", "reuse_allreduce"],
        default="fit_bcast",
        help="how to get (scale, L): fit_bcast (default) or reuse_allreduce",
    )
    args = ap.parse_args()

    repo = Path(__file__).resolve().parents[1]
    calibrate_bcast_one(repo, 2, args.mode)
    calibrate_bcast_one(repo, 3, args.mode)
    return 0
