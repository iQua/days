...
mod cuda_app {
    ...
    fn measure(
        ...
        let run = executor
            ...
            .unwrap_or_else(|error| {
                panic!("CUDA kind_clustering={kind_clustering} benchmark run failed: {error}")
            });
    ...
    fn checked(
        ...
        let (measurement, result) = measure(executor, image, kind_clustering);
        assert_eq!(
            &result, expected,
            "CUDA kind_clustering={kind_clustering} complete result differs from the warm baseline"
        );
        measurement
    }
    ...
    fn after_predecessor(
        ...
        let _ = checked(executor, image, kind_clustering, expected);
        checked(executor, image, kind_clustering, expected)
    }

    fn print_record(kind: &str, fixture: &str, measurement: Measurement) {
        println!(
            "record=t17c_kind_clustering_{kind} fixture={fixture} sample={} order={} \
    ...
    fn print_summary(fixture: &str, engine: &str, measurements: &[Measurement]) {
        ...
        println!(
            "record=t17c_kind_clustering_summary fixture={fixture} engine={engine} samples={} \
        ...
        for order in ["baseline_first", "candidate_first"] {
            let ordered = selected
                ...
                .collect::<Vec<_>>();
            println!(
                "record=t17c_kind_clustering_order_summary fixture={fixture} engine={engine} \
    ...
    pub fn main() {
        ...
        println!(
            "record=t17c_kind_clustering_protocol fixture={fixture} samples={SAMPLES} \
        ...
        for sample in 0..SAMPLES {
            ...
            let variants = if order == "baseline_first" {
                [false, true]
            } else {
                [true, false]
            };
            for kind_clustering in variants {
                let mut measurement =
                    after_predecessor(&executor, &image, kind_clustering, &expected);
        ...
        for engine in ["unmodified", "kind_clustered"] {
        ...
        let candidate = measurements
            .iter()
            .filter(|measurement| measurement.engine == "kind_clustered")
            .map(|measurement| measurement.backend_wall_ns)
            .collect::<Vec<_>>();
        ...
        let paired_wins = candidate
            ...
            .count();
        println!(
            "record=t17c_kind_clustering_formal fixture={fixture} \
             baseline_wall_ns={baseline_median} candidate_wall_ns={candidate_median} \
             baseline_ns_per_round={} candidate_ns_per_round={} \
             candidate_over_baseline={:.9} improvement_percent={:.6} \
